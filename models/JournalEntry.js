...
async function getTransactionPortionsForAccount(accountId){
    return (await pool.query(
        "SELECT journal_entry.*, financial_account.nickname as faccount_nickname, financial_account.category as faccount_category, transaction_portion.amount as amount, transaction_portion.description as description from journal_entry join transaction_portion on transaction_portion.journal_entry = journal_entry.id join financial_account on transaction_portion.financial_account = financial_account.id where financial_account.id = $1 order by journal_entry.for_date NULLS FIRST, transaction_portion.journal_entry, financial_account.category, financial_account.nickname", [accountId]
...
export default {getTransactionPortionsForUser, createJournalEntry, fillJournalEntry, getTransactionPortionsForAccount}