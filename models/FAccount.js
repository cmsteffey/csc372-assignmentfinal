...
async function getFAccounts(options){
    ...
    return (await pool.query(
        "SELECT financial_account.*, MIN(credit_card_info.default_cashback_pct) as default_cashback_pct, MIN(stock_financial_account.shares) as shares, MIN(stock_financial_account.ticker) as ticker, COALESCE(SUM(transaction_portion.amount), 0) as balance FROM financial_account left join stock_financial_account on stock_financial_account.financial_account = financial_account.id left join transaction_portion on transaction_portion.financial_account = financial_account.id left join credit_card_info on financial_account.id = credit_card_info.financial_account " + (options.userId ? "WHERE owner = $1 " : "") + (options.accountId ? "WHERE financial_account.id = " + (options.userId ? "$2 " : "$1 "): "") + "group by financial_account.id order by category, LOWER(nickname)"
